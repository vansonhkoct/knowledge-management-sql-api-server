...
llm: ChatGLM4 = ChatGLM4(
  llm_model_uses_gpu=True,
)
...
while (True):
    ...
    response_generator = llm.generator_answer(
      user_prompt=user_message,
      history=history,
    )

    for answer_result in response_generator:
        print(f"User: {answer_result.history[-1][0]}")
        print(f"Assistant: {answer_result.history[-1][1]}")
        print(f"LLM Output: {answer_result.llm_output()}")
        print(f"New Token: {answer_result.new_token}")
